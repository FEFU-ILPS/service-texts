...
def upgrade() -> None:
    ...
    op.create_table('learning_texts',
    sa.Column('id', sa.UUID(), nullable=False),
    sa.Column('title', sa.Text(), nullable=False),
    sa.Column('difficulty', sa.Integer(), nullable=True),
    sa.Column('value', sa.Text(), nullable=False),
    sa.Column('transcription', sa.Text(), nullable=False),
    sa.CheckConstraint('difficulty >= 0', name='check_difficulty_is_positive'),
    sa.PrimaryKeyConstraint('id'),
    sa.UniqueConstraint('title')
    )
    op.create_index('learning_text_title_idx', 'learning_texts', ['title'], unique=False, postgresql_using='hash')
